...
!$omp parallel do
         do i=1,nx
            do j=1,ny
               ...
               if ((abs(f(i,j+1)-mdv).gt.eps).and.
     >             (abs(f(i,j-1)-mdv).gt.eps)) then

                  df(i,j) = scale*(f(i,j+1)-f(i,j-1))
...
c           Scale factor for derivatives in m (latitude dependent)
            ...
            do i=1,nx
               ...
               if ((abs(f(i+1,j)-mdv).gt.eps).and.
     >             (abs(f(i-1,j)-mdv).gt.eps)) then

                  df(i,j) = scale*(f(i+1,j)-f(i-1,j))
